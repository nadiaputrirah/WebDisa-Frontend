...
export default function Populer({ search }) {
  const [data, setData] = useState([]);
  const getData = async () => {
    const result = await axios.get(
      `${process.env.REACT_APP_API}/destinasi?limit=3&search=${search}&isShow=true`,
      {
    ...
    <section class="bg-[#439A97] mb-20">
      <div class="py-8 px-4 mx-auto max-w-screen-xl sm:py-16 lg:px-6">
        ...
        <div class="w-full mb-20 space-y-8 md:grid md:grid-cols-2 lg:grid-cols-3 md:gap-12 md:space-y-0">
          {data.length == 0 ? (
            <p className="text-center">Data Yang Dicari Tidak ada...</p>
          ) : (
            data?.map((val) => {
              return (
                <div class="max-w-sm mx-auto p-8 bg-white border border-white-200 rounded-lg shadow">
                  <a href="/">
                    <img
                      class="rounded-t-lg"
                      src={`${process.env.REACT_APP_IMAGE_URL}/${val?.image_url}`}
                      alt=""
                    />
                  </a>
                  <div class="p-5">
